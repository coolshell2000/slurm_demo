...
test_containers_running() {
    ...
    # Check worker nodes (c1, c2, c3, etc.)
    WORKER_COUNT=0
    for i in {1..10}; do
        node="c$i"
...
test_compute_nodes() {
    ...
    NODE_COUNT=$(docker exec slurmctld sinfo -h -o "%n" | sort -u | wc -l)
...
test_resource_limits() {
    ...
    # Check specifically for the tiered setup: c1 (600MB) and c3 (267MB)
    C1_INFO=$(docker exec slurmctld scontrol show node c1 | grep -E "CPUTot|RealMemory")
    C3_INFO=$(docker exec slurmctld scontrol show node c3 | grep -E "CPUTot|RealMemory")

    if echo "$C1_INFO" | grep -q "RealMemory=600" && echo "$C3_INFO" | grep -q "RealMemory=267"; then
        print_pass "Resource limits (tiered memory) configured correctly"
    else
        print_fail "Resource limits not configured as expected"
        print_info "  c1 info: $C1_INFO"
        print_info "  c3 info: $C3_INFO"
        return 1
    fi
}
